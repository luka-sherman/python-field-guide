...
The count updates automatically as you add or remove items.
...
Negative indexes count down from the end, starting at `-1` for the last item. A slice `list[start:end]` returns a new list containing items from `start` up to (but not including) `end`.
...
`in` checks whether a value exists in a list at all.
{: .pt-subheading }
...
Assign to a slice to change a whole range at once — the replacement doesn't need to be the same length. `insert()` adds an item at a specific index without overwriting what's already there.
...
`insert()` adds one item at a chosen position. `extend()` adds every item from another iterable, one at a time — not the iterable itself.
...
`pop()` deletes an item by index and returns it — with no index, it removes the last item. `del` removes an item by index, or can delete the entire list. `clear()` empties the list but keeps the (now empty) list around.
...
`enumerate()` gives you the index and the value together, which is handy when you need both. A `while` loop works too, if you'd rather manage the index yourself.
...
Builds a new list from an existing iterable in a single line.
{: .pt-subheading }
...
Pass `reverse=True` to sort in the opposite order. `sorted()` does the same job but returns a new list, leaving the original untouched. A `key` function controls what each item is sorted by — here, by name length instead of alphabetically. `reverse()` is different from `sort(reverse=True)` — it just flips the current order in place, without sorting.
...
Writing `new_list = old_list` does **not** copy the list — both names point to the same list.
{: .pt-subheading }
...
A change through either name shows up in both. `copy()` (or `list()`, or a full slice `[:]`) makes a real, independent copy.
...
Tuples use the same index and slice syntax as lists.
{: .pt-subheading }
...
`0` for the first item, negative indexes count from the end, `start:end` slices out a sub-tuple.
...
Assigns each item in a tuple to its own variable in one line.
{: .pt-subheading }
...
The number of variables has to match the number of items. A [`match` statement](conditionals.md#unpacking-a-tuple) can do this same unpacking while also branching on the tuple's shape or specific values, which plain assignment can't do.
...
A tuple has no `append()` or `remove()` — but `+` builds a **new** tuple with an item added.
{: .pt-subheading }
...
Note the trailing comma in `("carpet",)`, needed to make it a one-item tuple rather than just parentheses around a string. For bigger changes, convert to a list with `list()`, edit it normally, then convert back with `tuple()`.
...
`get()` does the same thing, but returns `None` (or a default you choose) instead of raising an error if the key is missing. `keys()`, `values()`, and `items()` return view objects over the dictionary's keys, values, or key-value pairs. Use `in` to check whether a key exists at all.
...
`popitem()` removes and returns the last inserted key-value pair, as a tuple. `del` removes a key-value pair by key. `clear()` empties the dictionary but keeps the (now empty) dictionary around.
...
Loop over `.values()` to get just the values instead, or `.items()` to get both the key and the value together.
...
Writing `new_dict = old_dict` does **not** copy the dictionary — both names point to the same dictionary.
{: .pt-subheading }
...
A change through either name shows up in both. `copy()` (or `dict()`) makes a real, independent copy.
...
A dictionary's values can be other dictionaries.
{: .pt-subheading }
...
Useful for grouping related records under one variable, like a whole collection of snakes keyed by species. Chain square brackets to reach a value nested inside an inner dictionary.
